...
import { IdfEquation } from "@prisma/client";
...
export class IdfService {
  constructor(
    private readonly extendedDurationCalculator: ExtendedDurationCalculator,
    private readonly prisma: PrismaService,
  ) {}

  async getEquationById(id: string) {
  ...
  async findNearestStation(input: { latitude: number; longitude: number }) {
    if (!Number.isFinite(input.latitude) || !Number.isFinite(input.longitude)) {
      throw new NotFoundException("Latitude/longitude invalidas.");
    }
    ...
    if (!equations.length) {
      throw new NotFoundException("Nenhuma estacao com coordenadas disponivel.");
    }
  ...
  async calculate(input: CalculateIdfDto) {
    const requestedMode = (input.modo ?? "AUTO") as "LEGADO" | "MODERNO" | "AUTO";
    ...
    if (!selected) {
      ...
      if (!equation) {
        throw new NotFoundException("Nenhuma equacao encontrada para os filtros informados.");
      }
      selected = {
        equation,
    ...
    return {
      ...
      equacao: {
        ...
        equation_id: equation.equationId ?? (equation as IdfEquation).id ?? "INTERPOLADO",
  ...
  private async selectByCoordinateRule(input: {
    input: CalculateIdfDto;
    requestedMode: "LEGADO" | "MODERNO" | "AUTO";
    ...
    let localCandidate: IdfEquation | null = null;
    if (input.input.estacao) {
      ...
      if (selectedByText?.latitude && selectedByText?.longitude) {
        const selectedDistance = this.haversineMeters(
          input.input.latitude!,
          input.input.longitude!,
          selectedByText.latitude,
          selectedByText.longitude,
        );
    ...
    if (!localCandidate && nearest.distanceM <= input.localRadiusMeters) {
      const sameStation = equations.filter(
        (eq) =>
          this.normalizeText(eq.estacao) === this.normalizeText(nearest.equation.estacao) &&
    ...
    if (localCandidate) {
      return {
        ...
        distanceM: this.haversineMeters(
          input.input.latitude!,
          input.input.longitude!,
          localCandidate.latitude!,
          localCandidate.longitude!,
        ),
        aviso: null,
      };
    }

    const stationBuckets = new Map<string, IdfEquation[]>();
    ...
    const stationBest = [...stationBuckets.values()]
      ...
      .filter((eq): eq is IdfEquation => Boolean(eq))
      .map((eq) => ({
        equation: eq,
        distanceM: this.haversineMeters(
          input.input.latitude!,
    ...
    return {
      ...
      aviso:
        input.input.elevacaoM !== undefined
  ...
  private interpolateEquationByIdw(input: {
    points: Array<{ equation: IdfEquation; distanceM: number }>;
    power: number;
    uf: string;
    latitude: number;
    longitude: number;
  ...
  private async findBestMatch(input: {
    uf: string;
    municipio?: string;
    estacao?: string;
  ...
  private pickBestEquationForMode(equations: IdfEquation[], requestedMode: string) {
    if (!equations.length) {
      return null;
    }
    const ordered = [...equations].sort((a, b) => {
  ...
  private getSourceRank(sourceSheet: string | null) {
    const source = (sourceSheet ?? "").toLowerCase();
    if (source === "consolidado") return 0;
    if (source === "standard") return 1;
    if (source === "disaggregation") return 2;
    if (source === "interpolado") return 3;
    return 4;
  }

  private getSourceLabel(sourceSheet: string | null) {
    const source = (sourceSheet ?? "").toLowerCase();
    if (source === "consolidado") return "Atualizado";
    if (source === "standard") return "Legado";
    if (source === "disaggregation") return "Desagregacao";
    if (source === "interpolado") return "Interpolado";
  ...
  private getModeRank(modeValue: string | null, requestedMode: string) {
    ...
    if (requestedMode === "LEGADO") return mode === "LEGADO" ? 0 : 1;
    if (requestedMode === "MODERNO") return mode === "MODERNO" ? 0 : 1;
    if (mode === "MODERNO") return 0;
    if (mode === "LEGADO") return 1;
    return 2;
  }
